...
order by W desc;
--116 largest number win for SEA that did not win the world series
...
order by W; 
--83 smallest number of win for LAN that did win the world series
...
SELECT t.name,p.park_name,(SUM(attendance)/COUNT(games))as avg_attendance
